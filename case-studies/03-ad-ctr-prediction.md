...
- **Data parallelism** for the MLP layers: replicate the dense network on every worker, shard the training batch across them, and after each step every worker needs the *same* averaged gradient. The standard way to get there is **all-reduce** — specifically **ring all-reduce**, as implemented by NCCL/Horovod — where workers pass gradient shards around a logical ring until everyone holds the full average, with no single node acting as a bottleneck or a single point of failure.
- **Model parallelism** for the giant embedding tables: billions of user/ad/advertiser IDs don't fit on one device, so shard the table across machines instead of replicating it. This is naturally a **parameter-server** pattern instead — each server owns a slice of the table, workers send lookup/update requests only for the IDs in their current batch — because, unlike the dense gradient above, there's no "everyone needs everyone's update" step; a worker only ever touches the handful of rows its batch references.
- **Why two different communication patterns, not one:** all-reduce assumes every worker needs the full result, which is true for dense gradients but wildly wasteful for a table with billions of rows where any one batch touches a tiny fraction of them — you'd be all-reducing mostly zeros. DLRM's whole architecture is shaped by this split: dense compute is data-parallel over all-reduce, sparse embedding lookups are model-parallel over a parameter server (some systems swap the dedicated parameter-server tier for an **all-to-all shuffle** that exchanges looked-up rows directly between workers instead).
- **A third pattern, worth naming if pushed:** neither of the above helps when a single *dense* layer itself is too large for one device (common in giant transformer towers, less so in CTR models). That's **pipeline parallelism** (split by layer across devices) or **tensor parallelism** (split individual weight matrices across devices) — the serving-time version of this exact split shows up in [Q25](./25-model-serving-inference-system.md)'s discussion of partitioning oversized models. CTR towers rarely hit this regime, but naming it shows the all-reduce/parameter-server pair isn't the whole taxonomy.
...
- **All-reduce (ring all-reduce)** — a distributed-training communication pattern (implemented by libraries like NCCL/Horovod) where workers exchange gradient shards around a logical ring until every worker holds the same fully-averaged gradient, with no central bottleneck node; the standard way to synchronize a data-parallel dense network.
...
- **Parameter server** — a distributed-training architecture where dedicated server nodes hold shards of a model (typically a huge embedding table) and workers send lookup/update requests for only the rows their current batch touches, avoiding an all-reduce over data most workers never referenced.
- **pCTR** — predicted click-through rate: the model's estimate of P(click | user, ad, context).
- **Pipeline / tensor parallelism** — two ways to split a single oversized dense layer or network across devices when it doesn't fit on one: pipeline parallelism assigns whole layers to different devices, tensor parallelism splits individual weight matrices across devices. Rare in CTR-scale towers, standard in giant transformer training.
...
- [Horovod](https://github.com/horovod/horovod) — the reference framework for ring all-reduce data-parallel training across GPUs/hosts.
- [NCCL](https://developer.nvidia.com/nccl) — NVIDIA's collective-communication library implementing all-reduce and the other communication primitives Horovod and native PyTorch/TensorFlow distributed training sit on top of.

---

*Part of the [ML System Design Case Studies](../README.md) series. Connects to: [Q1](./01-video-recommendation.md) and [Q2](./02-news-feed-ranking.md) (shared retrieval/ranking and feedback-loop patterns), [Q9](./09-spam-abuse-detection.md) and [Q11](./11-real-time-fraud-detection.md) (extreme class imbalance, cost-sensitive metrics), [Q27](./27-data-labeling-active-learning.md) (exploration as active learning by another name), [Q25](./25-model-serving-inference-system.md) (this file's training-time all-reduce/parameter-server split has a serving-time analog in tensor/pipeline parallelism for oversized models).*
